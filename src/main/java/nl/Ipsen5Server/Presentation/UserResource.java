...
*/
    ...
    @PUT
    @Path("/{token}/password")
    public Response passwordUser(@PathParam("token") String token, Account user2) {
        ...
        try {
            ...
            Response successResponse = Response.ok(message).build();
    ...
    public Response loginUser(Account user) {
        ...
        try {
            ...
            Response successResponse = Response.ok(new TokenBody(token), MediaType.APPLICATION_JSON).build();
            response = successResponse; //change response
